...
const OPS: { op: string; description: string }[] = [
  ...
  },
  {
    op: 'pain-add',
    description:
      'Record an overheard user pain-point (read-only lurk harvest). --platform reddit|facebook|bluesky|other ' +
      '--pain-text "<one sentence>" [--vertical horeca|event|indie|fintech|other (def horeca) --user-segment ' +
      'venue-owner|event-planner|staff|indie-builder|other --severity high|med|low (def med) --rezerver-fit ' +
      'yes|maybe|no|na (def maybe) --source-ref "<sub/group + link>" --context-quote "<anonymized — NO username/PII>" ' +
      '--product-idea "<if a new opportunity>" --dedup-key "<reuse a known key to bump frequency>"]. ' +
      'Dedup by dedup-key: a re-seen pain increments frequency instead of duplicating.',
  },
  {
    op: 'pain-list',
    description:
      'List harvested pain-points, frequency-ranked. Optional --vertical --severity --status --platform --min-frequency <n>.',
  },
  {
    op: 'pain-set',
    description:
      'Triage a pain-point. --id <uuid> [--status new|triaged|actioned|dismissed --severity --rezerver-fit --product-idea].',
  },
  {
    op: 'pain-schema',
    description: 'List the settable fields for pain-add / pain-set.',
  },
];
