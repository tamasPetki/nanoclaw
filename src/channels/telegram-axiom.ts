/**
 * Telegram-Axiom channel adapter — dedicated Telegram bot for the autonomous
 * Axiom agent (ag-shift-lead, the HoReCa shift-staffing marketplace experiment
 * Lead/Architect) to report directly to Tomi. Mirrors telegram-hex.ts; differs
 * only in the env var (TELEGRAM_BOT_TOKEN_AXIOM) and channel-type ('telegram-axiom').
 */
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const existing = getMessagingGroupByPlatform('telegram-axiom', platformId);
      if (existing) {
...
registerChannelAdapter('telegram-axiom', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_AXIOM']);
    if (!env.TELEGRAM_BOT_TOKEN_AXIOM) return null;
    const token = env.TELEGRAM_BOT_TOKEN_AXIOM;
