...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    async function getCachedGroupMetadata(jid: string): Promise<GroupMetadata | undefined> {
      ...
      // Hand Baileys the RAW group metadata — participant ids in the group's
      // native addressing (LID under WhatsApp's LID migration, which baileys
      // 7.x uses). Do NOT rewrite participant ids to phone JIDs here: this
      // metadata feeds outbound group encryption / sender-key distribution, so
      // PN-normalized ids make baileys encrypt to identities the recipients'
      // clients don't use for the group → messages are server-acked but
      // undecryptable (invisible "waiting for this message"). Inbound sender
      // resolution uses translateJid() separately and must not leak into here.
      // Downstream fix vs fork/channels:src/channels/whatsapp.ts (adapter was
      // written for baileys ^6.x PN-addressed groups) — re-apply if recopied.
    ...
    async function connectSocket(): Promise<void> {
      ...
      // 'chats.phoneNumberShare' is emitted by baileys 7.x at runtime but isn't in
      // the pinned rc.9's typed BaileysEventMap, so cast the listener loosely. This
      // is a best-effort mid-session refinement; the primary LID→phone mapping is
      // already established from sock.user.lid on connection above. Downstream patch
      // vs fork/channels:src/channels/whatsapp.ts — re-apply if the adapter is recopied.
      (sock.ev.on as unknown as (ev: string, cb: (a: { lid?: string; jid?: string }) => void) => void)(
