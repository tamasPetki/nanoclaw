/**
 * Telegram-Worker channel adapter — dedicated Telegram bot for the autonomous
 * worker agent (ag-worker) to report directly to Tomi, instead of relaying
 * through the hub. Mirrors telegram.ts; differs only in the env var
 * (TELEGRAM_BOT_TOKEN_WORKER) and channel-type ('telegram-worker').
 */
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const existing = getMessagingGroupByPlatform('telegram-worker', platformId);
      if (existing) {
...
registerChannelAdapter('telegram-worker', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_WORKER']);
    if (!env.TELEGRAM_BOT_TOKEN_WORKER) return null;
    const token = env.TELEGRAM_BOT_TOKEN_WORKER;
