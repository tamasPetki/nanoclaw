/**
 * Telegram-Friend channel adapter — a dedicated Telegram bot that lets the
 * autonomous Axiom agent (ag-shift-lead) interview an external design partner
 * (Tomi's musician-manager friend, the Kept v0 user) directly, on a separate
 * channel from Tomi's own DM so the two threads never mix. Mirrors
 * telegram-axiom.ts; differs only in the env var (TELEGRAM_BOT_TOKEN_FRIEND)
 * and channel-type ('telegram-friend').
 */
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const existing = getMessagingGroupByPlatform('telegram-friend', platformId);
      if (existing) {
...
registerChannelAdapter('telegram-friend', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_FRIEND']);
    if (!env.TELEGRAM_BOT_TOKEN_FRIEND) return null;
    const token = env.TELEGRAM_BOT_TOKEN_FRIEND;
