/**
 * Telegram-Hex channel adapter — dedicated Telegram bot for the autonomous Hex
 * agent (ag-tracker, HeadlessTracker solo-dev) to report directly to Tomi,
 * instead of relaying through the hub. Mirrors telegram.ts; differs only in the
 * env var (TELEGRAM_BOT_TOKEN_HEX) and channel-type ('telegram-hex').
 */
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const existing = getMessagingGroupByPlatform('telegram-hex', platformId);
      if (existing) {
...
registerChannelAdapter('telegram-hex', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_HEX']);
    if (!env.TELEGRAM_BOT_TOKEN_HEX) return null;
    const token = env.TELEGRAM_BOT_TOKEN_HEX;
