/**
 * Telegram-Ledger channel adapter — a dedicated Telegram bot for the autonomous
 * "Ledger" product-incubator team (ag-ledger), which builds an internal financial
 * management app (QuickBooks-alternative) for Tomi's own companies. Ledger (the
 * lead) reports to Tomi on this bot; its two specialists (Tally = finance domain
 * + integrations, Form = design/UX) are A2A-only and have no bot of their own.
 * Mirrors telegram-axiom.ts / telegram-friend.ts; differs only in the env var
 * (TELEGRAM_BOT_TOKEN_LEDGER) and channel-type ('telegram-ledger').
 */
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const existing = getMessagingGroupByPlatform('telegram-ledger', platformId);
      if (existing) {
...
registerChannelAdapter('telegram-ledger', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_LEDGER']);
    if (!env.TELEGRAM_BOT_TOKEN_LEDGER) return null;
    const token = env.TELEGRAM_BOT_TOKEN_LEDGER;
