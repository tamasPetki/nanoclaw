/**
 * Telegram-Radar channel adapter — dedicated Telegram bot for the autonomous
 * Radar agent (competitor intelligence) to report directly to Tomi, instead of
 * relaying through the hub. Mirrors telegram-hex.ts; differs only in the env var
 * (TELEGRAM_BOT_TOKEN_RADAR) and channel-type ('telegram-radar').
 */
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const existing = getMessagingGroupByPlatform('telegram-radar', platformId);
      if (existing) {
...
registerChannelAdapter('telegram-radar', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_RADAR']);
    if (!env.TELEGRAM_BOT_TOKEN_RADAR) return null;
    const token = env.TELEGRAM_BOT_TOKEN_RADAR;
