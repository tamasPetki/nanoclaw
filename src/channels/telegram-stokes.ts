/**
 * Telegram-Stokes channel adapter — second Telegram bot for Szandra (wife) to
 * reach the Stokes butler agent. Mirrors telegram.ts; differs only in the env
 * var (TELEGRAM_BOT_TOKEN_STOKES) and channel-type ('telegram-stokes').
 */
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      const existing = getMessagingGroupByPlatform('telegram-stokes', platformId);
      if (existing) {
...
registerChannelAdapter('telegram-stokes', {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_STOKES']);
    if (!env.TELEGRAM_BOT_TOKEN_STOKES) return null;
    const token = env.TELEGRAM_BOT_TOKEN_STOKES;
