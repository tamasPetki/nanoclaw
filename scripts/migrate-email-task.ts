...
const emailPrompt = [
  ...
  '',
  '3. ⚠️ NINCS KÁRTYA, NINCS `send_email`. Minden draftként megy a Piszkozatokba — Tomi küldi a kliensében. KÖTELEZŐ: olvasd vissza az `email-assistant` SKILL.md "Draft mentése a Piszkozatokba" + "Email aláírás" + "Számla-továbbítás a pénzügyes felé" szekcióit MIELŐTT draftolsz. (A blokkoló kártya mire Tomi a Telegramhoz ült, timeoutolt — Tomi 2026-06-01 kivetette.)',
  '',
  '   **pietscarlet — DRAFT a Piszkozatokba** (a skill workflow-ja szerint):',
  '   a) Válasz-igénylő email → teljes HTML válasz (köszönés "Szia <Név>! 😊", érdemi tartalom, záró "Köszönöm szépen! Szép napot!", teljes PietScarlet HTML-aláírás). Tilalmak: NINCS "Tomi kérésére továbbítom", NINCS utasítás a partnernek, HTML kötelező (lásd a hub CLAUDE.local.md draft-szabályát).',
  '   b) Számla (díjbekérő, proforma, "Számla érkezett", szamlazz.hu, PDF-csatolmány) → továbbítás-draft Erikának (penzugy@pietscarlet.hu) "Fwd: " tárggyal, kedves rövid szöveggel + az eredeti tartalommal; a PDF-et `download_attachment`-tel töltsd le és `--attach`-csal tedd rá. Erikának NEM kell a PietScarlet aláírás-kártya.',
  '   c) Mentés: `python3 /workspace/agent/email-save-draft.py pietscarlet "<címzett>" "<tárgy>" /tmp/draft-<uid>.html "<Message-ID ha válasz>" [--attach /tmp/szamla-<uid>.pdf]`. A script `INBOX.Drafts`-ba teszi `\\Draft` flaggel → Tomi kliensében a Piszkozatok közt. Verify: `status=OK`.',
  '',
  ...
  '',
  '4. Telegram summary (sima Markdown szöveg, NEM card, NEM blokkoló): fiókonként mi jött (db, prio). A pietscarletnél jelezd melyik emailhez/számlához tettél draftot ("X válaszához és Y számlájához (Erikának) draftot tettem a pietscarlet Piszkozatokba -- nézd meg és küldd"). A lupaobol/trinkenessen válasz-igénylő itemeit is listázd, hogy Tomi tudjon róluk.',
