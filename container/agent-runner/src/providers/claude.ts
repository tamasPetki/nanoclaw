...
function createPreCompactHook(assistantName?: string): HookCallback {
  return async (input) => {
    ...
    // Auto-wiki-extract: scan user messages for project mentions and append
    // them to wiki/log.md before the compact summary erases the detail.
    // No-ops on non-wiki groups (file missing → return). Best-effort.
    if (transcriptPath && fs.existsSync(transcriptPath)) {
      try {
    ...
    // Archive the full transcript to conversations/ (shared helper, also used
    // by maybeRotateContinuation on cold-resume).
    archiveTranscriptFile(transcriptPath, sessionId, assistantName);
