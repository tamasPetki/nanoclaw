...
// 8 min (not 5): heavy configs (opus-4-8[1m] + xhigh effort + large context)
// can legitimately go token-silent for >5 min mid-generation, and a false
// abort there truncates an in-flight message. 8 min still recovers well
// before the host-sweep 30-min ceiling.
const STREAM_SILENCE_ABORT_MS = 8 * 60 * 1000;
