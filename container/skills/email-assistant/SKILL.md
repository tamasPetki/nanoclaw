...
2. Írd a HTML body-t egy fájlba (a quote-ok és többsoros HTML így sértetlenül átmegy):
   ...
   <p>&nbsp;</p>
   <p>Üdvözlettel,</p>
   ... + aláírás (lásd "Email aláírás" szekció) ...
   HTML
   ```
3. Mentsd draftként:
   ```bash
...
2. Csoportosítsd őket:
   ...
   - **Phishing-gyanú**: lásd a kötelező szűrőt lent

   **⚠️ KÖTELEZŐ phishing / feladó-hitelesség szűrő (MIELŐTT bármit a summaryba írsz):**
   Ha egy email témája **fiók/biztonság-érzékeny** — kulcsszavak: *account restriction /
   suspension / deletion / verify / verification / login / password / unusual activity /
   „fiók felfüggesztve / törölve / igazold / erősítsd meg"*, cPanel/hosting/Microsoft/Google/
   bank/Meta-Facebook márka, „Business Manager partner request", stb. — akkor **NE jellemezd
   és NE kösd más eseményhez** (pl. egy disk-alerthez), amíg le nem ellenőrizted a feladó
   hitelességét:
   1. Kérd le a **nyers headert** (`get_emails_content` raw / IMAP) és nézd a
      **`Return-Path` / envelope-from** értékét, valamint a `Received` láncot.
   2. Hasonlítsd a látható `From:` domainhez. Ha az **envelope-from idegen domain** (nem a
      cég saját domainje), vagy a `From` spoofolt-nak tűnik, vagy blacklist-jel van
      (HOSTKARMA-BLACK, SPF/DKIM fail) → **phishing-gyanú**.
   3. A summaryban **phishing-ként** jelöld (ne tippelj „valós okot"), és ajánld fel a
      spam-mappába mozgatást (`move_emails` → `INBOX.spam`). NE készíts választ-draftot,
      NE kövess linket.
   4. Bizonytalan eset → kérdezd meg Tomit, ne jellemezd valósként.

   *Háttér (2026-06-12 incidens):* egy cPanel „Temporary Account Restriction" levelet tévesen
   a napi disk-alerthez kötöttem a summaryban; Tominak kellett rákérdeznie hogy „nem scam?".
   A nyers header `Return-Path: support@scshlxy.com` (idegen, blacklisten) → phishing volt.
   Ne ismétlődjön.
...
1. **Fogalmazz választ** Tomi nevében, az alábbi elvek mentén:
   - Hangnem: **alapból formális és visszafogott** — magázó megszólítás, üzleties,
     professzionális stílus, ha nincs kialakult, közvetlenebb viszony. Csak ott tegezz /
     válts közvetlen hangra, ahol a partner a korábbi levelezésben EGYÉRTELMŰEN ezt
     használja (építőipari, régóta futó kapcsolatok). Kétség esetén magázz.
   - **Kifelé menő levélben ne tegyél emojit** (😊, 👍 stb.) — a hangnemet szavakkal add
     meg, ne hangulatjellel. (Ez a Telegram-summary emojikra NEM vonatkozik, csak a
     partnernek kimenő email-szövegre.)
   - Lényegre törő, nem túl hosszú; kerüld a felesleges bevezető udvariaskodást.
   - **Tagolt, levegős tördelés — ÜRES SORRAL, nem CSS-margóval:** minden érdemi bekezdés
     külön `<p>`-ben, és a bekezdések KÖZÉ tegyél egy `<p>&nbsp;</p>` üres bekezdést valódi
     sorköznek. **NE inline `margin`/`line-height`-tal próbáld a térközt** — a Spark (és több
     kliens) a szerkeszthető törzsből kiszedi a CSS-t, így a margó nem látszik; az üres
     bekezdés viszont tartalom, ezért megmarad. A köszönés, az érdemi bekezdések és a záró
     formula is külön `<p>`, mindegyik közé egy `<p>&nbsp;</p>`. Ne tömöríts több gondolatot
     egy bekezdésbe. (Az aláírás külön ügy: az a `data-spark-custom-html` miatt érintetlen.)
...
### Egyéb szabályok
- **Alapból formális; a partner kialakult hangnemét tiszteld.** Új vagy tisztázatlan
  viszonynál magázz, professzionális hangon. Ha a partner a korábbi levelezésben
  egyértelműen tegez, maradj annál — de a default a magázás.
...
Az email body-t mindig HTML-ként formázd (`html: true`). A záró sor és az aláírás közé
tegyél egy `<br>`-t elválasztásnak. Az aláírás az alábbi **kanonikus block — másold be
EXAKTUL, ne improvizálj, és NE told vissza a régi `<p>&nbsp;</p>` előblokkot vagy a
wrapper `margin-top`-ot** (a block ezeket szándékosan már nem tartalmazza):
            ...
            <tr>
                ...
                <td style="padding: 0 0 0 16px; vertical-align: middle;">
                    ...
                    <div style="margin-top: 2px; font-size: 12px; line-height: 16px; color: #625851;">
                        ügyvezető &nbsp;·&nbsp; <span style="font-weight: 700;">PietScarlet Kft.</span>
                    </div>
...
- A záró sor és az aláírás közé egy `<br>` elég az elválasztáshoz. **NE** told vissza a
  régi `<p>&nbsp;</p>` előblokkot vagy a wrapper `margin-top: 24px;`-et — a kanonikus
  block ezeket szándékosan nem tartalmazza.
...
- A draft HTML body-jába a **teljes** HTML aláírás kerüljön (a fenti block egészében) —
  Tomi a kliensében renderelt aláírással látja és küldi. Ne rövidítsd "[aláírás blokk]"
